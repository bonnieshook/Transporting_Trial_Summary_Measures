...
# Purpose: Simulation study for transportability paper (hazard ratio scenarios): Combine Sim results and output table 
...
Sc1.mod<-as.data.frame(read.csv("01_simulation_results_scen1_HR_meta_mod.csv"))
Sc2.mod<-as.data.frame(read.csv("02_simulation_results_scen2_HR_meta_mod.csv"))
Sc3.mod<-as.data.frame(read.csv("03_simulation_results_scen3_HR_meta_mod.csv"))
Sc4.mod<-as.data.frame(read.csv("04_simulation_results_scen4_HR_meta_mod.csv"))
Sc5.mod<-as.data.frame(read.csv("05_simulation_results_scen5_HR_meta_mod.csv"))

Sc1.strong<-as.data.frame(read.csv("01_simulation_results_scen1_HR_meta_strong.csv"))
Sc2.strong<-as.data.frame(read.csv("02_simulation_results_scen2_HR_meta_strong.csv"))
Sc3.strong<-as.data.frame(read.csv("03_simulation_results_scen3_HR_meta_strong.csv"))
Sc4.strong<-as.data.frame(read.csv("04_simulation_results_scen4_HR_meta_strong.csv"))
Sc5.strong<-as.data.frame(read.csv("05_simulation_results_scen5_HR_meta_strong.csv"))
...
write.csv(all_scen, "04_SimSummary_table_HR_primary.csv")
...
# save to a text file
writeLines(Latex.tab, "Latex.tab.HR.prim.txt")
...
#Run for each scenario
Sc1.mod.res_naive<-prep_data_naive(Sc1.mod,1,truth_list_mod[[1]])
Sc2.mod.res_naive<-prep_data_naive(Sc2.mod,2,truth_list_mod[[2]])
Sc3.mod.res_naive<-prep_data_naive(Sc3.mod,3,truth_list_mod[[3]])
Sc4.mod.res_naive<-prep_data_naive(Sc4.mod,4,truth_list_mod[[4]])
Sc5.mod.res_naive<-prep_data_naive(Sc5.mod,5,truth_list_mod[[5]])

Sc1.strong.res_naive<-prep_data_naive(Sc1.strong,1,truth_list_strong[[1]])
Sc2.strong.res_naive<-prep_data_naive(Sc2.strong,2,truth_list_strong[[2]])
Sc3.strong.res_naive<-prep_data_naive(Sc3.strong,3,truth_list_strong[[3]])
Sc4.strong.res_naive<-prep_data_naive(Sc4.strong,4,truth_list_strong[[4]])
Sc5.strong.res_naive<-prep_data_naive(Sc5.strong,5,truth_list_strong[[5]])

all_scen_naive<-as.data.frame(rbind(Sc1.mod.res_naive,Sc2.mod.res_naive,Sc3.mod.res_naive,Sc4.mod.res_naive,Sc5.mod.res_naive,Sc1.strong.res_naive,Sc2.strong.res_naive,Sc3.strong.res_naive,Sc4.strong.res_naive,Sc5.strong.res_naive))
colnames(all_scen_naive)<-c("Scenario","Estimator","Bias (%)", "Relative Bias (%)", "ASE", "ESE", "SER", "Coverage","CI Overlap")
write.csv(all_scen_naive, "04_SimSummary_table_HR_primary_naive.csv")


all_scen_naive<- all_scen_naive %>% select(-c("CI Overlap"))

all_scen_naive<- all_scen_naive %>%
  mutate(Scenario = case_when(
    Scenario == "1" ~ "2A",
    Scenario == "2" ~ "2B",
    Scenario == "3" ~ "2C",
...
Latex.tab_naive <- apply(all_scen_naive, 1, function(x) {
  paste0(" & ", paste(x, collapse = " & "), " \\\\")
})

# save to a text file
writeLines(Latex.tab_naive, "Latex.tab.HR.prim.naive.txt")
