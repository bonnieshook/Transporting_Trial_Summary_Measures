################################################################################
# Program: 01_sims.R
# Purpose: Simulation study for transportability paper; code to run sims for all scenarios - risk ratio
...
one_trial <- function(t) {
  ...
  dat.trial <- tibble(part_id = seq_len(num.part.Trials)) %>%
    mutate(
      ...
      lp_Y0 = log(0.20) + log(1.8) * X + log(1.6) * U,
...
#Helper function - run a single meta analysis and return the RR and se(RR)
meta_analysis <- function(data) {
  META<-metabin(events_A1, n_A1, events_A0, n_A0, data=data, studlab=R, sm="RR", method="MH")
  META.SUM<-summary(META)
  Meta.logRR<-(META.SUM$TE.common)
  Meta.selogRR<-META.SUM$common$seTE
  res<-c(Meta.logRR,Meta.selogRR)
return(res)
}
...
# Conduct meta-analysis and extract MH estimated RR and standard error (marginal and conditional)
META.marg<-meta_analysis(all.trials.marg)
META.cond.X0<-meta_analysis(all.trials.cond[all.trials.cond$X==0,])
META.cond.X1<-meta_analysis(all.trials.cond[all.trials.cond$X==1,])
